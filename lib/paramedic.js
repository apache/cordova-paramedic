...
class ParamedicRunner {
    ...
    async runLocalTests () {
        ...
        let testResults = utilities.TEST_PASSED;
        ...
        if (this.shouldWaitForTestResult()) {
            testResults = await Promise.race([
                this.waitForTests(), // resolve on request
                this.waitForConnection() // reject on timeout
            ]);
        }
    ...
    waitForTests () {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.server.on('jasmineDone', (data) => {
                logger.info('[paramedic] tests have been completed');
