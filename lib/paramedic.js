...
class ParamedicRunner {
    ...
    run () {
        ...
        const isAndroid = this.config.getPlatformId() === utilities.ANDROID;

        const shouldReversePort = (
            isAndroid &&
            (target !== undefined && !target.startsWith('emulator-'))
        );
        ...
        return Q().then(() => {
            ...
            .then(async (localServer) => {
                if (shouldReversePort) {
                    console.log('[paramedic] Setting "adb reverse"');
                    await utilities.setupReversePort(localServer.port, target);
                }
                return localServer;
            })
            .catch(err => {
            ...
            .then((server) => {
                if (this.config.runMainTests()) {
                    ...
                    const logUrl = this.server.getMedicAddress(this.config.getPlatformId(), shouldReversePort);
