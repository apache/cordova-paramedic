...
/**
 * Sets up ADB reverse port rules so an Android physical device can connect
 * to the Medic server running on the host machine. This is required for real
 * devices.
 *
 * @param {number} port - The host port running the Medic server.
 * @param {string} target - The ADB device ID
 * @returns {Promise<void>}
 */
function setupReversePort (port, target) {
    return new Promise((resolve, reject) => {
        const cmd = 'adb';
        const args = ['-s', target, 'reverse', `tcp:${port}`, `tcp:${port}`];
        const proc = spawnSync(cmd, args, { encoding: 'utf8' });

        console.log(`[paramedic] Running command: ${cmd} ${args.join(' ')}`);

        if (proc.error) {
            return reject(new Error(`[paramedic] Failed to spawn with error: ${proc.error.message}`));
        }
        if (proc.status !== 0) {
            return reject(
                new Error(
                    `[paramedic] Failed to setup adb reverse port. (exit ${proc.status}): ${proc.stderr || 'Unknown error'}`
                )
            );
        }
        return resolve();
